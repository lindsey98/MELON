"""ChatInject attacks (https://github.com/hwanchang00/ChatInject).

The exploit reconstructs the target model's chat template *inside* an injected tool
output: interrupt tags close the current turn and open a fake one of another role, so
a system/user/assistant exchange the attacker authored is parsed as genuine
conversation history. See ``MODEL_CONFIGS`` for the per-family delimiters.

Two families of attack are registered:

* Template-only (``chat_inject_qwen3`` / ``chat_inject_glm``): a single fake
  system+user+assistant turn ending in an empty think block, filled with the
  injection ``{goal}``. Works on any suite.
* Multi-turn (``chat_inject_*_with_utility_*_multiturn_7``): prepend a pre-generated
  multi-turn "utility" dialogue, loaded from ``chatinject_data/`` keyed by injection
  GOAL. ChatInject only generated this data for the banking / slack / travel GOALs, so
  an uncovered GOAL raises ``ValueError``.
"""
