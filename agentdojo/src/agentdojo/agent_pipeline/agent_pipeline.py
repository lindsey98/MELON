...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "melon":
            # MELON re-executes the agent's trajectory with a masked user prompt
            # and compares the resulting tool calls to detect prompt injection.
            # The MELON element wraps the LLM (it calls ``self.llm.query`` for
            # both the original and masked runs), so it replaces the LLM at the
            # end of the tools loop.
            tools_loop = ToolsExecutionLoop(
                [
                    ToolsExecutor(tool_output_formatter),
                    MELON(llm),
